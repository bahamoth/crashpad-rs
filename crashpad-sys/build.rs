...
"#,
    ...
    println!("cargo:rustc-link-search=native={}", obj_dir.join("minidump").display());
    println!("cargo:rustc-link-search=native={}", obj_dir.join("snapshot").display());
    println!("cargo:rustc-link-search=native={}", obj_dir.join("handler").display());
    ...
    let ar_status = match target_os.as_str() {
        "macos" | "ios" => {
            ...
            Command::new("libtool")
                .args(&["-static", "-o", lib_path.to_str().unwrap(), wrapper_obj.to_str().unwrap()])
                .status()
    ...
    if !handler_path.exists() {
        println!("cargo:warning=crashpad_handler not found at expected location: {}", handler_path.display());
    }
